...
export default function ExcelUpload({ onQuizCreated }: ExcelUploadProps) {
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/quizzes/parse-excel', {
        method: 'POST',
        body: formData,
      })
      ...
      setError('An error occurred while parsing Excel file')
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !classId || parsedQuestions.length === 0) {
      setError('Please fill in all required fields and parse the Excel file')
      return
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
        <h3 className="font-medium text-blue-900 mb-2">Excel Template Format:</h3>
        <ul className="text-sm text-blue-800 space-y-1">
          ...
          <li>Column F: Correct Answer (A/B/C/D)</li>
